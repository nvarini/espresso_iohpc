...
MODULE wstat_restart
  ...
  CONTAINS
    !
    !------------------------------------------------------------------------
    SUBROUTINE wstat_restart_write( dav_iter, notcnv, nbase, ew, hr_distr, vr_distr )
      !------------------------------------------------------------------------
      !
      ...
      USE xml_io_base,          ONLY : attr,create_directory
    ...
    !
    !------------------------------------------------------------------------
    SUBROUTINE wstat_restart_read( dav_iter, notcnv, nbase, ew, hr_distr, vr_distr)
      !------------------------------------------------------------------------
      !
      ...
      !
      CALL read_restart3_( dirname, hr_distr, vr_distr )
      !
    ...
    !
    !------------------------------------------------------------------------
    SUBROUTINE read_restart3_( dirname, hr_distr, vr_distr )
      !------------------------------------------------------------------------
      !
    ...
    SUBROUTINE read_restart4_( dirname, nbase )
      ...
      INTEGER :: iun
      !
      IF(.NOT.ALLOCATED(dvg)) ALLOCATE(dvg(npwx,pert%nlocx))
      IF(.NOT.ALLOCATED(dng)) ALLOCATE(dng(npwx,pert%nlocx))
      dvg = 0._DP
      dng = 0._DP
      !
      DO local_j=1,pert%nloc
