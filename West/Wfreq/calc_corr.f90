...
!
!-----------------------------------------------------------------------
SUBROUTINE calc_corr( sigma_corr, energy )
  !-----------------------------------------------------------------------
  !
  ...
  USE pwcom,                ONLY : npw,npwx,et,nks,current_spin,isk,xk,nbnd,lsda,igk,g2kin,ecutwfc,nspin,current_k,wk
  USE westcom,              ONLY : qp_bandrange,isz,&
                                 & nbnd_occ,l_enable_lanczos,&
                                 & n_lanczos,iks_l2g,l_macropol,&
                                 & ihead,rhead,ibody1,ibody2,idiago,rbody
     ...
     DO ib = qp_bandrange(1), qp_bandrange(2)
        ...
        IF(l_macropol) THEN
           ...
           DO ifreq = 1,ifr%nloc
              ...
              partial_h = partial_h + ihead(ifreq)*integrate_imfreq(ifreq,enrg)
           ENDDO
           !
        ENDIF
        !
        ...
        DO ifreq = 1,ifr%nloc
           DO im = 1, aband%nloc
              ...
              partial_b = partial_b + ibody1(im,ifreq,ib,iks)*integrate_imfreq(ifreq,enrg)
           ENDDO
        ENDDO
        !
        ...
        IF( l_enable_lanczos ) THEN
           ...
           DO ifreq = 1,ifr%nloc
              DO ip = 1, pert%nloc
                 DO il = 1, n_lanczos
                    enrg = idiago(il,ip,ib,iks) - energy(ib,iks)
                    partial_b = partial_b + ibody2(il,ip,ifreq,ib,iks)*integrate_imfreq(ifreq,enrg)
                 ENDDO
              ENDDO
           ENDDO
           !
        ...
        !
        CALL update_bar_type( barra, 'sigmac_i', 1 )
        !
     ENDDO ! ib
     !
  ENDDO ! iks
  !
  CALL stop_bar_type( barra, 'sigmac_i' )
  !
  ! -------------------
  ! The part with poles
  ! -------------------
  ...
  DO iks = 1, nks
     ...
     DO ib = qp_bandrange(1), qp_bandrange(2)
        ...
        DO im = 1,aband%nloc
           ...
           IF( this_is_a_pole ) THEN
              ...
              DO ifreq = 1, rfr%nloc 
                 ...
                 !
                 IF(glob_im==ib.AND.l_macropol) residues_h = residues_h + segno * rhead(ifreq)
                 !
                 residues_b = residues_b + segno * rbody( im, ifreq, ib, iks )
                 ! 
              ENDDO
              !
           ENDIF 
        ...
        !
        CALL update_bar_type( barra, 'sigmac_r', 1 )
        !
     ENDDO ! ib  
     !
  ENDDO ! ik
  !
  CALL stop_bar_type( barra, 'sigmac_r' )
  !
END SUBROUTINE
!
!
...
SUBROUTINE retrieve_glob_freq( freq, glob_ifreq )
  ...
  ! Find glob_ifreq
  !
  glob_ifreq = 1 + NINT( REAL(n_refreq-1,KIND=DP) * (freq + ecut_refreq) / 2._DP / ecut_refreq ) 
