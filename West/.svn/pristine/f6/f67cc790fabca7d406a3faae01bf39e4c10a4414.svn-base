...
!
!-----------------------------------------------------------------------
SUBROUTINE calc_exx( sigma_exx, nb1, nb2 )
  !-----------------------------------------------------------------------
  !
  ...
  CALL io_push_bar()
  !
  ALLOCATE( pertg( npwx*npol ) )
  ALLOCATE( pertr( dffts%nnr ) )
  ALLOCATE( mysqvc( npwq0 ) )
  !
  CALL store_sqvc(mysqvc,npwq0,div_kind_hf,mydiv)
  !
  ! Set to zero
  !
  sigma_exx = 0._DP
  ...
  DO iks = 1, nks   ! KPOINT-SPIN
     ...
     DO ib = nb1, nb2
        ...
        DO iv = 1, vband%nloc
           ...
           ! Bring it to R-space
           IF(gamma_only) THEN
              CALL SINGLEBAND_invfft(npwq0,evc(1,iv_glob),npwx,pertr,dffts%nnr)
              DO ir=1,dffts%nnr 
