...
MODULE pdep_io
  ...
  USE westcom,     ONLY : npwq0, npwq0_g, q0ig_l2g
  USE wvfct,       ONLY : npwx 
  !
  IMPLICIT NONE
  !
  CONTAINS
    ...
    SUBROUTINE pdep_merge_and_write_G(fname,pdepg)
      ...
      COMPLEX(DP), INTENT(IN) :: pdepg(npwx)
      !
      ! Scratch
      !
      ...
      tmp_vec=0._DP
      !
      CALL mergewf( pdepg(:), tmp_vec, npwq0, q0ig_l2g, me_bgrp, nproc_bgrp, root_bgrp, intra_bgrp_comm) 
      !
    ...
    SUBROUTINE pdep_read_G_and_distribute(fname,pdepg)
      ...
      !    
      CHARACTER(*), INTENT(IN) :: fname
      COMPLEX(DP), INTENT(OUT) :: pdepg(npwx)
      !
      ! Scratch
      !
      COMPLEX(DP),ALLOCATABLE :: tmp_vec(:)
      ...
      IF(my_pool_id==0.AND.my_bgrp_id==0) THEN
         ...
         END IF
         !
         CALL splitwf( pdepg, tmp_vec, npwq0, q0ig_l2g, me_bgrp, nproc_bgrp, root_bgrp, intra_bgrp_comm) 
         !
      ENDIF
      !
      DEALLOCATE( tmp_vec )
